...
(function() {
  ...
  var prefixedFullscreenEnabled: string = null;
  [
    'fullscreenEnabled',
   ...
   * @param callback
   */
  function loadWebVTTFile(url: string, callback) {
   ...
   * @returns {number}
   */
  function getCurrentChapter(cues: Array, currentTime: number): number {
    ...
    videos.forEach(function(video, index) {
      ...
      function updateChapterCues(event) {
        if (!video.textTracks || !video.textTracks[0] ||
          playlist[index].chapterCues.length > 0) {
          return;
        }
   ...
   * @returns {Object}
   */
  function videoSrcElement(id: string = null, src: string = null, video: HTMLVideoElement = null, label: string = null) {
    return {
      id: id,
      src: src,
      video: video,
  ...
  xtag.register('x-video', {
    ...
    methods: {
      ...
      addTextTrack: function(kind: string, label: string = undefined, language: string = undefined) {
       ...
       */
      playByID: function(elementID: string) {
        ...
        var targetElementIndex: number = null;
       ...
       */
      playChapter: function(chapterIndex: number) {
